...
<body>
	...
	<?php 
    	...
    	$queryfp = "SELECT username from follow where fusername = '$username'";

    	$resultfp = pg_query($queryfp) or die('Query failed: ' . pg_last_error());

    	$tempfp="The users you are following:";
    	echo "<div class='d02'>
		...
		while ($linefp = pg_fetch_array($resultfp, null, PGSQL_ASSOC)) {	
			...
			foreach ($linefp as $col_valuefp) {
				...
				onclick='document.getElementById(";echo '"formfollow';echo $i; echo '"';echo ").submit();'>
				<br>$col_valuefp</p>";
				echo "<form id='formfollow$i' action='followplaylist.php' method = 'post'>
				<input name=username type='hidden' value='";echo $username;echo "'>
				<input name=upassword type='hidden' value='";echo $upassword;echo "'>
				...
				echo "
	    			...
	    			<form id='formfollow$i' action='unfollow.php' method = 'post'>
					<input name=username type='hidden' value='";echo $username;echo "'>
					<input name=upassword type='hidden' value='";echo $upassword;echo "'>
					<input name=fusername type='hidden' value='";echo $col_valuefp;echo "'>
