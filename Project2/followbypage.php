...
<body>
	...
	<?php 
    	...
    	$queryfp = "SELECT fusername from follow where username = '$username'";

    	$resultfp = pg_query($queryfp) or die('Query failed: ' . pg_last_error());

    	$tempfp="You are followed by:";
    	echo "<div class='d02'>
    	<h2 style='color: #108BAF;'>$tempfp</h2>";
    	
		$i=0;
		while ($linefp = pg_fetch_array($resultfp, null, PGSQL_ASSOC)) {	
			
			foreach ($linefp as $col_valuefp) {
				echo "<p style='font-size: 18px; font-family: Georgia'>
				<br>$col_valuefp</p>";
				
				$querytest2="select * from follow where username in (select username from follow where fusername='$username') and username ='$col_valuefp' and username <> fusername";
				...
				echo "
	    			...
	    			<form id='formfollow$i' action='tofollow.php' method = 'post'>
					<input name=username type='hidden' value='";echo $username;echo "'>
					<input name=upassword type='hidden' value='";echo $upassword;echo "'>
					<input name=fusername type='hidden' value='";echo $col_valuefp;echo "'>
