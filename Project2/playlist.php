...
/*
			...
			while ($line = pg_fetch_array($resultx, null, PGSQL_ASSOC)) {
			    ...
			    foreach ($line as $col_value) {
			        ...
			        if($i==0){
			        	$query1="select trackid, trackname,albumid,trackartist
			        	from playlist natural join playlisttrack natural join track natural join album
			        	where pid = '$col_value'";
			        	...
			        	echo "<table>\n";
						echo "<tr><th>No</th><th>Track</th><th>Album</th><th>Artist</th><th>Option</th><th>Option</th></tr>";
			        	$k=0;
			        	while($line1 = pg_fetch_array($result1, null, PGSQL_ASSOC)){
			        		...
			        		foreach ($line1 as $col_value1) {
			        			if($j==1){
						        	...
						        	$t=$k+1;
						        	$temp1=pg_fetch_result($result2, $k, 0);
						        	
						        }
						        if($j==2){
						        	$queryalbum="select albumname from album where albumid='$col_value1'";
									//$queryalbum=pg_escape_string($queryalbum);
									$resultalbum=pg_query($queryalbum) or die('Query failed: ' . pg_last_error());
									$bufferalbumname=pg_fetch_result($resultalbum, 0, 0);
						        	echo '<td onclick=document.getElementById("form';echo $t;
									echo '").submit() style="cursor: pointer;">';echo $bufferalbumname;'</td>';
									echo '<form id="form';
									echo $t;
									echo '" action="Album.php" method = "post">
										<input name="gotoalbum" type="hidden" value="';
									
									echo $bufferalbumname;
						        ...
						        if($j==3){
									...
									?><td onclick="document.getElementById('artistform<?php echo $a,$k;?>').submit();" style="cursor: pointer"><?php echo $col_value1; ?></td><?php
						        	echo '<form id="artistform';echo $a,$k;echo '" action="artist.php" method = "post">';
									echo '<input name="username" type="hidden" value="';echo $username;echo '">';
									echo '<input name="upassword" type="hidden" value="';echo $upassword;echo '">';
									echo '<input name="artisttitle" type="hidden" value="';echo $col_value1;echo '">';
									echo '</form>';
						        	...
						        	echo '<td><li class="dropdown">
							                <a id="';echo $a,$k;echo '" href="javascript:void(0)" class="dropbtn" onclick="showList(this)">Add to Playlist</a>
							                <div class="dropdown-content" id="dropdown-';echo $a,$k;echo '" onmouseleave="hideList(this)">';
							                $d=0;
			...
			echo "<h2>Following User's Playlist ";
			echo '</h2>';

			$query_follow="select username from follow where fusername='$username'";
			$result_follow=pg_query($query_follow) or die('Query failed: ' . pg_last_error());
			while($linef = pg_fetch_array($result_follow, null, PGSQL_ASSOC)){
	    		$f=0;
	    		foreach ($linef as $col_valuef) {
	    			$tempuser=$col_valuef;
	    	echo "<h2>$tempuser's Playlist ";
			echo '</h2>';

			$queryx="select pid, ptittle from playlist where username='$tempuser' ";
			$resultx=pg_query($queryx) or die('Query failed: ' . pg_last_error());
			...
			while ($line = pg_fetch_array($resultx, null, PGSQL_ASSOC)) {
			    ...
			    foreach ($line as $col_value) {
			        ...
			        if($i==0){
			        	...
			        	echo "<table>\n";
						echo "<tr><th>No</th><th>Track</th><th>Album</th><th>Artist</th><th>Option</th></tr>";
			        	$k=0;
			        	while($line1 = pg_fetch_array($result1, null, PGSQL_ASSOC)){
			        		$j=0;
			        		foreach ($line1 as $col_value1) {
